...
def upload_admit_card(request):
    if request.method == 'POST':
        session_year = request.POST.get('session_year')
        semester = request.POST.get('semester')
        exam_date = request.POST.get('exam_date')
        course_id = request.POST.get('course_id')
        admit_card_file = request.FILES.get('admit_card_file')
        ...
        eligible_students = Students.objects.filter(
            Q(session_year_id=session_year) & Q(course_id=course_id)
        )

        # Associate eligible students with the examination
        examination.students.set(eligible_students)

        return redirect('success_page')
    return render(request, 'upload_admit_card.html')
